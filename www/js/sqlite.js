...
sqlite.factory('eventosFactory', function ($q, $cordovaSQLite) {
    return {
        insert: function (id, dsCategoria, dsTitulo, dsNoticia, dtNoticia, dsUrlImagem, flLido) {
            var query = "INSERT INTO eventos (id, dsCategoria, dsTitulo, dsNoticia, dtNoticia, dsUrlImagem, flLido) VALUES (?, ?, ?, ?, ?, ?, ?)";
            var values = [id, dsCategoria, dsTitulo, dsNoticia, dtNoticia, dsUrlImagem, flLido];
            var outputs = [];
            //Usada para fazer o retorno do banco aguardar esta completa
            var deferred = $q.defer();
        ...
        selectNoticia: function (id) {
            var query = "SELECT  id, dsCategoria, dsTitulo, dsNoticia, strftime('%d-%m-%Y %H:%M:%S', datetime(dtNoticia)) as dtNoticia, dsUrlImagem, flLido FROM eventos WHERE id=?";
            var values = [id];
            var outputs = [];
        ...
        selectListaNoticias: function () {
            var query = "SELECT id, dsCategoria, dsTitulo, strftime('%d-%m-%Y %H:%M:%S', datetime(dtNoticia)) as dtNoticia, dsUrlImagem FROM eventos ORDER BY date(dtNoticia) DESC";
            var outputs = [];
        ...
        verificaNoticia: function (id) {
            var query = "SELECT 1 as retorno FROM eventos WHERE id=?";
            var values = [id];
            var outputs = [];
